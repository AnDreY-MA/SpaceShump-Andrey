...
/// <summary>
/// Enemy4 создается за верхней границей, выбирает случайную точку на экране
...
public class Enemy4 : Enemy
{
    ...
    void Start()
    {
        ...
        //поэтому записать ее как начальные значения в p0 и p1
        p0 = p1 = pos;
        InitMovement();
    ...
    bool Destroyed(Part prt)
    {
        ...
        //Вернуть результат сравения
        //Если prt.health <= 0 — true
        return (prt.health <= 0);
    }

    //Окрашивает в красный только одну часть
        ...
        switch(other.tag)
        {
            case "ProjectileHero":
                ...
                if(prtHit.protectedBy != null)
                {
                    ...
                    {
                        //Если хотя бы одна из защищающих часте ещё не разрушена
                        if(!Destroyed(s))
                        {
                            //не наносить повреждений это части
                            Destroy(other);
                            return;
                        }
                    }
                ...
                //Эта часть не защищена, нанести ей вореждение
                //Получить разрушающию силу из Projectile.type и Main.Weap_DICT
                ...
                if (prtHit.health <= 0)
                {
                    //Вмксто разрушения всего корабля, деактевировать уничтоженную часть
                ...
                if(allDestroyed)
                {
                    //Если корабль разрушен полностью, уведомить объект-одиночку Main
