...
public class Main : MonoBehaviour
{
    ...
    {
        S = this;
        //Записать в bndCheck ссылку на компонент BounndsCheck этого игрового объекта
        bndCheck = GetComponent<BoundsCheck>();
        //Вызывать SpawnEnemy() один раз(в 2 секунды)
    ...
    public void SpawnEnemy()
    {
        ...
        //Разместить вражеский корабрль над экраном в случайной позиции x
    ...
    public void Restart()
    {
        //Перезагрузить Scene0, чтобы перезапустить игру
        SceneManager.LoadScene("SampleScene");
    }

    static public WeaponDefinition GetWeaponDefinition(WeaponType wt)
    {
        //Проверять наличие указанного ключа в словаре
        if (WEAP_DICT.ContainsKey(wt))
        {
            return (WEAP_DICT[wt]);
        }

        //Возвращение нового экземпляра WeaponDefinition с типом оружия WeaponType.none
